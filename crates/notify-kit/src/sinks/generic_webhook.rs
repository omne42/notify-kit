...
use crate::sinks::http::{
    build_http_client, build_http_client_pinned_async, parse_and_validate_https_url_basic,
    redact_url, redact_url_str, sanitize_reqwest_error, validate_url_path_prefix,
};
    ...
    fn send<'a>(&'a self, event: &'a Event) -> BoxFuture<'a, anyhow::Result<()>> {
        Box::pin(async move {
            let client = if self.enforce_public_ip {
                build_http_client_pinned_async(self.timeout, self.url.clone()).await?
            } else {
                self.client.clone()
            };
